...
{
    Emulator::Emulator()
        : m_cpu(m_bus), m_ppu(m_pictureBus, m_emulatorScreen), m_screenScale(3.f), m_cycleTimer(),
    ...
    void Emulator::run(std::string rom_path)
    {
        ...
        m_mapper = Mapper::createMapper(
            ...
            [&]() { m_cpu.interrupt(InterruptType::IRQ); }, [&]() { m_pictureBus.updateMirroring(); });
        if (!m_mapper)
        {
        ...
        m_window = smk::Window(NESVideoWidth * m_screenScale, NESVideoHeight * m_screenScale, "SimpleNES");

        m_emulatorScreen.create(NESVideoWidth, NESVideoHeight, m_screenScale, smk::Color::White);
