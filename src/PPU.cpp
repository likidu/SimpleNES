...
namespace sn
{
    PPU::PPU(PictureBus &bus, VirtualScreen &screen)
        : m_bus(bus), m_screen(screen), m_spriteMemory(64 * 4),
          m_pictureBuffer(ScanlineVisibleDots, std::vector<smk::Color>(VisibleScanlines, smk::Color::Magenta))
    {
    }
            ...
            if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots)
            {
                ...
                if (m_showSprites && (!m_hideEdgeSprites || x >= 8))
                {
                    for (auto i : m_scanlineSprites)
                    {
                        ...
                        {
                            // bit-3 is one if it is the bottom tile of the sprite, multiply by two to get the next
                            // pattern
                ...
                m_pictureBuffer[x][y] = smk::Color(colors[m_bus.readPalette(paletteAddr)]);
            }
